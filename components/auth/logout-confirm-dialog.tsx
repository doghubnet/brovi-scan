...
export function LogoutConfirmDialog({ email, loading, onCancel, onConfirm }: { email?: string | null; loading: boolean; onCancel: () => void; onConfirm: () => void }) {
  ...
  return (
    <motion.div initial={reduced ? undefined : { opacity: 0 }} animate={reduced ? undefined : { opacity: 1 }} className="fixed inset-0 z-50 grid place-items-center bg-navy/60 px-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="logout-title" aria-describedby="logout-description">
      <motion.div ref={dialogRef} initial={reduced ? undefined : { opacity: 0, scale: 0.98, y: 8 }} animate={reduced ? undefined : { opacity: 1, scale: 1, y: 0 }} transition={{ duration: reduced ? 0 : 0.2 }} className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-6 shadow-glow dark:border-white/10 dark:bg-navy">
