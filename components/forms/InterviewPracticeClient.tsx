...
type FormValues = { targetCountry: string; visaType: string; difficulty: string; category: string; question: string; answer: string };
...
export function InterviewPracticeClient() {
  ...
  const { register, handleSubmit, setValue, watch, formState: { errors, isSubmitting } } = useForm<FormValues>({ defaultValues: { difficulty: "Normal", category: "General", question: interviewQuestions[0] } });
  const countryOptions = getActiveCountryOptions();
  ...
  function previousQuestion() { const prev = (index - 1 + interviewQuestions.length) % interviewQuestions.length; setIndex(prev); setValue("question", interviewQuestions[prev]); setValue("answer", ""); setFeedback(null); }
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_380px]">
      <form className="card space-y-4" onSubmit={handleSubmit(submit)}>
        <div className="grid gap-4 md:grid-cols-3">
          <label className="label">Country selection<select className="input mt-2" {...register("targetCountry", { required: true })}><option value="">Select country</option>{countryOptions.map((c)=><option key={c.value} value={c.label}>{c.label}</option>)}</select>{errors.targetCountry ? <span className="text-xs text-risk">Country is required.</span> : null}</label>
          ...
          <label className="label">Category selector<select className="input mt-2" {...register("category")}><option>General</option><option>Study plan</option><option>University choice</option><option>Program choice</option><option>Sponsor and finance</option><option>Future plan</option></select></label>
        </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row"><button className="btn-primary" disabled={isSubmitting} type="submit">{isSubmitting ? "Checking..." : "Get answer feedback"}</button><button className="btn-secondary" type="button" onClick={previousQuestion}>Previous question</button><button className="btn-secondary" type="button" onClick={nextQuestion}>Next question</button></div>
      </form>
      <aside className="card h-fit">{feedback ? <><ScoreRing score={feedback.score} label="Interview Readiness Score" /><h3 className="mt-6 text-lg font-bold">Feedback</h3><RecommendationList items={[`Clarity: ${feedback.strengths[0] ?? "Needs work"}`,`Specificity: ${feedback.strengths[1] ?? "Needs work"}`,`Consistency: ${feedback.weaknesses[0] ?? "Review details"}`,`Financial clarity: ${feedback.weaknesses[1] ?? "Check funding explanation"}`,`Honesty: keep explanations factual and consistent.`,`Improved answer: ${feedback.recommendations[0] ?? "Add specific evidence."}`,`What to practice next: ${feedback.recommendations[1] ?? "Continue one-question practice."}`]} /><p className="mt-4 rounded-xl bg-amber-50 p-3 text-sm text-amber-800 dark:bg-amber-400/10 dark:text-amber-100">{feedback.warning}</p></> : <p className="muted">Submit an answer to receive feedback and build your practice history.</p>} {saveMessage ? <p className="mt-4 text-xs muted">{saveMessage}</p> : null}{history.length ? <div className="mt-5"><h3 className="font-bold">Answer history</h3><ul className="mt-2 space-y-1 text-sm muted">{history.map((item, idx) => <li key={idx}>Practice {history.length - idx}: {item.score}%</li>)}</ul></div> : null}</aside>
    </div>
  );
}
