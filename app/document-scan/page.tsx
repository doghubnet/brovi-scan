...
const fields = docs.map(([label,name]) => ({ label: `${label} ⓘ`, ariaLabel: `${label}. ${helperTip}`, title: helperTip, name, options: documentStatuses }));

export default function Page() {
  const [country, setCountry] = useState("");
  const pack = useMemo(() => (country ? getCountryPack(country) : null), [country]);
  return <DashboardShell><h1 className="section-title">Document Scan</h1><p className="muted">Sections: Identity, Academic, Admission, Financial/Sponsor, Translation/Legalization, and country-specific checks.</p><p className="mt-2 rounded-xl bg-slate-100 p-3 text-sm dark:bg-white/5">Select the current status for each document. Official requirements must be verified before submission.</p><CountrySelector value={country} onChange={setCountry} includePlanned />{pack ? <div className="space-y-3"><OfficialVerificationAlert message={pack.requiredVerificationMessage} />{pack.status === "active" ? <CountryChecklistGroups pack={pack} /> : <div className="card text-sm">Country pack planned, verify official source.</div>}</div> : <div className="card text-sm">No country selected yet. Generic checklist is shown below.</div>}<AttachmentUploadPanel /><DocumentUploadRules /><ScanForm fields={fields} endpoint="/api/analyze-documents" submitLabel="Review documents" tableName="document_reviews" /></DashboardShell>;
}
